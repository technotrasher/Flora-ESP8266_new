...
void ICACHE_RAM_ATTR TimerHandler()
{
  ...
  // Доступен только один таймер прерывания (ISR), поэтому, чтобы точки не мерцали и не сбоили во время подключения к Wi-Fi, нам нужно разместить его здесь...
  // Скорость движения точек зависит от частоты обновления дисплея.
  if (enableDotsAnimation) { // анимация бегающих точек в начале запуска
  ...
  shiftWriteBytes(bytes); // Цифры записаны в обратном порядке (первый сдвиговый регистр соответствует последней цифре и т. д.).
