...
/** Перенаправлять на captive-портал при получении запроса для другого домена. В этом случае возвращать true, чтобы обработчик страниц не пытался снова обработать запрос.. */
...
void startConfigPortal() {
  configStartMillis = millis(); // start counter на 5 минут отдыхаем
  ...
  /* Настройте DNS-сервер так, чтобы он перенаправлял все домены на apIP. */
  ...
  while (deviceMode == CONFIG_MODE) { // Блокирующий бесконечный цикл

    time_t remainingSeconds = (CONFIG_TIMEOUT - (millis() - configStartMillis)) / 1000;

    if (millis() - configStartMillis > CONFIG_TIMEOUT) { // 5 минут закончились
      WiFi.softAPdisconnect(true); // точка доступа отключается
      //WiFi.mode(WIFI_STA); // устройство подключается к существующему роутеру
      deviceMode = CONNECTION_FAIL; // если истекло время и ничего не введено
      Serial.println("проверка на подключение к ви-фи. данные не введены");
      return;
    }
...
void startServer() {
  ...
  server.on("/generate_204", handleRoot);  //Captive-портал в Android. Возможно, не требуется. Эту задачу может взять на себя обработчик `notFound`
  server.on("/fwlink", handleRoot);  //Captive-портал Microsoft. Возможно, он не нужен. Эту задачу может взять на себя обработчик notFound.
  //server.on("/json", handleJson); // Показать необработанный JSON по указанному пути URL.
...
void handleNotFound() {
  if (captivePortal()) { // Если вместо страницы с ошибкой происходит перенаправление на страницу авторизации (captive portal).
    return;
  }
...
void handleRoot() {
  ...
  if (!server.args() || (server.args() && server.hasArg("is_form"))) {
    ...
    /* // Дополнительные настройки для статического IP-адреса; в некоторых случаях это может сократить время отклика.
    ...
    */
    ...
    server.sendContent(""); // Это сообщает веб-клиенту, что передача завершена.
